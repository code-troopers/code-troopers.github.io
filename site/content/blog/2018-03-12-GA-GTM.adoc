...
Je ne m'attarde pas sur cette étape qui est assez simple et déjà très bien documenté.
...
Normalement pour ajouter *Google Analytics* à votre application, vous auriez dû (de la même manière qu'on a ajouté *GTM* juste avant), suivre les instructions de *Google Analytics* à la création du projet sur celui-ci et copier-coller un bout de code pour l'activer, mais vous l'avez compris, ce n'est pas ce qu'il va se passer vu qu'on ne va plus toucher au code.
...
Vous pouvez par exemple déclarer un tableau de variables qui seront différentes suivant les environnements.
