...
Un des points génant lors de l'utilisation de Jenkins est le coté volatile de la configuration des jobs de builds.
...
Ensuite, c'est là que la magie opère, plutot que de devoir sélectionner les n-items voulus et remplir chaque étape du build, nous pouvons maintenant le décrire en utilisant du code !
...
Il est intéressant de noter que l'image Docker qui sert au build est une image personnalisée. Ce n'est pas parce qu'elle inclu un quelconque fonctionnement permettant de builder
